...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.5):
        ...
        self.act = act_layer()
        self.drop = nn.Dropout(drop)
        # self.fc1 = DWConv2d(in_features, hidden_features, 1, 1)
        # self.fc2 = DWConv2d(hidden_features, out_features, 1, 1)
...
class PATM(nn.Module):
    def __init__(self, dim, qkv_bias=False, qk_scale=None, attn_drop=0.5, proj_drop=0.5, mode='fc'):  # 本来是0.3
        super().__init__()
        ...
        self.proj_drop = nn.Dropout(proj_drop)
        self.m = nn.Sequential(*(Bottleneck(dim, dim, shortcut=True, g=1, e=1.0) for _ in range(2)))
        self.mode = mode

        if mode == 'fc':
            self.theta_h_conv = nn.Sequential(nn.Conv2d(dim, dim, 1, 1, bias=True),
                                              nn.BatchNorm2d(dim),
                                              nn.ReLU())
            self.theta_w_conv = nn.Sequential(nn.Conv2d(dim, dim, 1, 1, bias=True),
                                              nn.BatchNorm2d(dim),
                                              nn.ReLU())
        else:
...
class ASFFV7(nn.Module):
    def __init__(self, level, multiplier=1, rfb=False, vis=False, act_cfg=True):
        ...
        self.level = level
        self.dim = [int(1024*multiplier), int(768*multiplier), 
                    int(512*multiplier), int(256*multiplier)]
        self.inter_dim = self.dim[self.level]
        if level == 0:
            ...
            self.compress_level_0 = Conv(
                int(1024*multiplier), self.inter_dim, 1, 1)
            self.stride_level_2 = Conv(
                int(512*multiplier), self.inter_dim, 3, 2)
            ...
            self.compress_level_0 = Conv(
                int(1024*multiplier), self.inter_dim, 1, 1)
            self.compress_level_1 = Conv(
                int(768*multiplier), self.inter_dim, 1, 1)
            self.stride_level_3 = Conv(
                int(256*multiplier), self.inter_dim, 3, 2)
            self.expand = Conv(self.inter_dim, int(512*multiplier), 3, 1)
        elif level == 3:
    ...
    def forward(self, x): #l,m,s
        ...
        if self.level == 0:
            ...
            level_2_downsampled_inter = F.max_pool2d(
                x_level_2, 3, stride=2, padding=1)
            level_2_resized = self.stride_level_2(level_2_downsampled_inter)
            # level_3_downsampled_inter_1 = F.max_pool2d(
            #     x_level_3, 3, stride=2, padding=1)
            # level_3_downsampled_inter_2 = F.max_pool2d(
            #     level_3_downsampled_inter_1, 3, stride=2, padding=1)
            # level_3_resized = self.stride_level_3(level_3_downsampled_inter_2)
            level_3_downsampled_inter = F.max_pool2d(
                x_level_3, 3, stride=4, padding=1)
...
class WaveBlock(nn.Module):
    ...
    def __init__(self, dim, mlp_ratio=2., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
